...
- X 长文正文插图必须走 `X Article` Markdown inline image 流，或拆成每条图文绑定的 thread；普通 X 主帖多图只会变成 gallery，不能当正文插图使用。
- `x-browser.ts` 默认拒绝长文本 + 多图 + submit，除非显式 `--allow-longform-gallery` 且发布清单写明 gallery 是故意的。
- Codex / GPT Image 生成图可能带 OpenAI C2PA / Content Credentials，X 会显示“由 AI 生成”；不要通过剥离 provenance 隐藏 AI 来源，若要避免该标签应使用真实截图、人工设计图或授权素材。
