...
3. 如果用户要求看 X timeline，必须优先读取或生成 `05-选题研究/X-timeline-fresh-following-YYYY-MM-DD.md`：默认目标 100 条当前 following chronological timeline，并按当天日期过滤；样本不足时必须在结论里标明缺口，不能把 home/for-you 样本或 20 条互动队列当成完整 today timeline 观察。
