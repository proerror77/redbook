...
06-业务运营/          # 商业数据
wiki/                 # LLM 维护的知识库（见下方 Wiki Schema）
  ├── index.md        # 目录索引
...
1. **AI 只做初稿，不做成稿**
   - 默认把 AI 当作提纲、初稿、改写建议、结构助手
   - 不把 AI 直接输出当成最终发布版本
2. **AI 占比控制在低位**
   - 默认目标：最终可见成稿中的 AI 原样表达尽量控制在 `25%-30%` 以内
   - 这不是平台公开硬阈值，而是保守工作线；如果超过这一线，必须加大人工改写和素材嵌入力度
3. **人工打磨优先级高于继续生成**
   - 遇到“内容很顺、信息很多、但不像自己会说的话”，优先改，不要继续让 AI 扩写
4. **分段生成，分段改**
   - AI 每次只生成 `300-500` 字
   - 每段生成后立刻人工改写，再进入下一段
...
3. 提供封面文字建议
4. 如果是小红书图文，额外做一次 **点击承诺一致性检查**：
   - 标题、封面文案、正文前两屏必须回答同一个问题
   - 不允许标题讲 A、封面讲 B、正文再讲 C
5. 如果小红书第一钩子里出现 `Prompt`、`Harness`、`工作流`、`工程化` 这类抽象词，默认再补一版“具体场景 + 明确结果”的改写版
...
#### 「情绪密度检查」
当用户在做任何平台文稿时，默认增加一轮情绪密度检查：
1. AI 稿件常见问题不是没信息，而是没情绪牵引
2. 默认检查每 `100-150` 字是否至少有一个情绪反应点：
   - 反差
   - 共鸣
   - 痛点戳破
   - 意外判断
   - 让人想停住的一句短句
3. 如果整段内容只有信息，没有情绪起伏，优先人工重写，不继续补信息
...
#### 「发布前内容风控（强制）」
任何内容在点击发布前，必须额外过这 5 道门：
1. **AI 痕迹检查**
   - 人工回读一遍，确认不像“直接从 AI 里复制出来”
   - 如果本地已安装 `dbskill`，优先用 `/dbs-ai-check` 做一轮诊断
2. **同质化检查**
   - 对比最近 `7` 天该平台的热门内容
   - 确认标题、结构、开头、关键表达，不要和近期热门稿高度重合
   - 平台真正重罚的往往不是“用了 AI”，而是“和别人写得太像”
3. **点击承诺一致性检查**
   - 标题、封面文案、正文前两屏必须承诺同一件事
   - 如果用户点进来后发现内容实际在讲另一个问题，默认算高风险稿
   - 小红书第一钩子优先写具体问题或具体结果，不要先讲抽象概念
4. **标题与敏感词检查**
   - 检查标题是否合规、是否过度夸张、是否踩平台敏感词
   - 检查正文是否包含明显风险词、营销黑词、触发审核的表达
5. **暗限流风险检查**
   - 检查内容是否同时命中：AI 味重、模板感重、情绪密度低、结构撞车
   - 如果这 4 项里同时命中 2 项以上，默认不上线，先重写
