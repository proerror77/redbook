---
name: zhipin
description: 使用普通已登录 Chrome + Computer Use 慢速监督进行 BOSS 直聘 live apply；tools/auto-zhipin 负责筛选、账本、计数和诊断，CDP/Browser Trace 只作为显式诊断 fallback。
---

# /zhipin

这个 Skill 是 BOSS 工作流的入口说明。当前 live apply 主链是用户普通已登录 Chrome + Computer Use 慢速监督。`tools/auto-zhipin` 继续负责筛选、账本、计数、doctor 和非 live gate；current-tab/CDP / Browser Trace 只作为显式诊断入口，不作为默认真实投递路径。
...
- Position: BOSS is an explicit exception: newly launched CDP/automation profiles are high-risk for live apply. Prefer normal visible Chrome + Computer Use for real applications.

当前主入口：
- 真实投递：普通已登录 Chrome + Computer Use 慢速监督
- 扫描/抽职位/本地候选：`npm run chrome:collect`
- 单条非 live 预检：`npm run boss:apply -- --url <job-url> --dry-run true`
- 消息监看：`npm run chrome:monitor`
- 报告：`npm run report`

`boss:apply-current`、CDP、Browser Trace 只在用户明确接受风险时用于诊断或 dry-run；不要用新开的 `--remote-debugging-port` Chrome 做 BOSS live apply。`boss:apply-opencli` / OpenCLI / OpenSeal 只保留为版本和 doctor 诊断参考，不用于 BOSS chat、search 或 apply 主流程。
...
2. 用 Computer Use 慢速查看 Chat、排除已拒绝和重复投递。
3. 用 Computer Use 在现有页面/推荐流中筛选岗位，脚本只做账本和本地 gate。
