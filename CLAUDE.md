...
- `选题决策门` 固定输出：新闻/选题一句话、账号主线契合、建议形态、为什么不是其他形态、推荐角度、是否需要小红书企业翻译、下一步动作。
...
5. 输出 3-5 个可写选题，每个使用 `选题决策卡`：热度、争议点、账号契合、建议形态、推荐角度、为什么不建议其他形态。
...
4. `/x-mastery-mentor` 快速审稿：算法层、Hook 层、事实风险、链接/回复结构。
5. 展示最终稿、审稿结论和发布结构，等用户确认发布。
6. 发布后保存 `X短评.md` / `发布记录.md`，并记录状态 URL。
