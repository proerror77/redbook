...
- 每日 X 运营必须先生成 `X-timeline-sample-YYYY-MM-DD.md` / `.json`：从当前 X home timeline 读取目标 100 条原始样本，作为“今天发生了什么”的第一证据；再生成 `X-互动队列-YYYY-MM-DD.md`，从原始样本里筛 20 条高互动人/帖和回复草稿。默认只生成候选和草稿，不自动发布评论。
...
2. 如果用户要求看 X timeline，必须优先读取或生成 `05-选题研究/X-timeline-sample-YYYY-MM-DD.md`：默认目标 100 条当前 home timeline 原始样本；样本不足 100 条时必须在结论里标明缺口，不能把 20 条互动队列当成完整 timeline 观察。
...
8. 同步生成每日 X 互动队列：优先从 100 条 timeline 原始样本中挑有回复/转帖/喜欢势能、且能接上账号主线的人和帖子，生成 20 条回复候选；实际发布回复时必须按 `x-engagement-reply-workflow` 复筛和审稿。
9. 不创建完整 content run，不写发布清单，不自动写 `00-选题记录.md`。

完成标准：
- 有推荐选题列表。
- 有来源报告或当前浏览器证据；如果 X 证据不可用，结论必须标明不是来自 X timeline；如果 timeline 原始样本少于 100 条，必须标明样本不足。
- 明确哪些题适合 X、哪些适合小红书或长文。
- 有 `X-timeline-sample-YYYY-MM-DD.md` 和 `X-互动队列-YYYY-MM-DD.md`，或明确说明 X timeline 证据/浏览器不可用导致样本/互动队列缺口。
...
- 每日 X timeline 样本与互动队列：`tools/redbookctl daily` 默认先生成目标 100 条 `X-timeline-sample-YYYY-MM-DD.md`，再筛 20 条 `X-互动队列-YYYY-MM-DD.md`；单独调试用 `python3 tools/auto-x/scripts/build_engagement_queue.py --source timeline --timeline-sample-size 100 --limit 20`
