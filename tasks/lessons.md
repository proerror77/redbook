...
- 问题：共享薪资 Gate 用年化区间下限比较，导致上限达到目标、且岗位方向匹配的机会在完整 JD 之前被过早排除。
- 根因：把“保守保底薪资”当成了当前投递策略；用户的新策略实际是“薪资区间上限达到 30K 即可进入后续筛选”。
- 修正动作：共享 `evaluateJob()` 改用年化区间上限比较；活动配置的 filters/apply 两处阈值统一为 30K。
- 预防规则（Rule）：薪资策略变化必须同时验证共享函数、filters 配置和 apply 覆盖值；不得只改面板或单一路径。
- 下次触发信号：`20-40K`、`salary_below_minimum`、用户说放宽薪资或优先沟通机会。
- 验证结果：`20-40K` Gate 探针返回 allow；聚焦测试 40/40、全量 348/348；重启后今日成功数从 9 增至 17，包含多条 `20-40K` 完整 JD/LLM 终审通过岗位。

### Lesson 124
- 日期：2026-08-24
- 场景：BOSS Userscript 在完成 7 条受监督检查点后没有继续，用户指出历史日志已记录平台每天 150 次沟通额度。
- 问题：把临时检查点 `7/7` 写进 `dailySuccessfulAppliesTarget`，同时持久化为站点暂停，导致 Userscript 自身虽保留 150 上限，Gate 仍在第 7 条永久阻断。
- 根因：混淆了“本轮监督检查点”和“平台每日额度”两个状态；检查点结束不应改写每日总上限。
- 修正动作：Gate 每日上限恢复为 150；小批次监督仅用于回读进度，不再写成 `checkpoint_complete_*` 站点限制。
- 预防规则（Rule）：每日额度只由平台提示或用户明确配置决定；批次检查点不得修改 `dailySuccessfulAppliesTarget`，也不得持久化为站点风控暂停。
- 下次触发信号：面板显示 `N/N`、`daily_apply_cap_reached` 或 `checkpoint_complete_*`，但 N 只是本轮批次数。
- 验证结果：Gate 重启并读回 `dailyApplyCap=150`；恢复后浏览器面板显示 `8/150`、投递权限已开启，并新增一条完整 JD/LLM 终审后成功落账的投递。

### Lesson 123
- 日期：2026-04-29
- 场景：X 三条内容已生成配图并进入发布包，但实际发出时三条都只有文字，用户指出“一个严重的错误，都没有图”。
- 问题：
  1) 发布链路把 X 文案当成唯一输入，没有把同内容包下的 `cover.png` 作为必须附带资产
  2) 第一次补救时曾依赖编辑器状态/发布动作，风险是出现“图片已传但正文未真正进入最终帖”的错配
  3) 验证如果只看“已发送”提示或状态 URL，仍可能漏掉图文完整性
- 根因：
  1) 发布前缺少“文案 + 配图资产 + 最终帖可见状态”的三点核对
  2) 对 X 富文本编辑器不能只相信自动化控件值，必须以最终个人主页/状态页为准
  3) 没把用户要求的小红书配图素材同步提升为 X 图文发布的硬约束
- 修正动作：
  1) 三条旧文字版删除前先分别完成图文重发
  2) 每条新帖都在 Chrome `Shih` profile 的 `@0xcybersmile` 主页验证正文和 `图像` 链接
  3) 删除三条旧文字版后，再回到主页确认顶部三条都是图文帖
- 预防规则（Rule）：
  1) X 内容包内只要存在 `cover.png` / `card_*.png`，发布前必须进入图文链路；不得降级成纯文字帖
  2) X 发布完成标准必须同时满足：状态链接存在、正文可见、`photo/1` 或 `图像` 链接可见
  3) 对 X 编辑器不要用单一 `set_value`/控件状态作为最终证据；最终证据只能来自个人主页或状态页
- 下次触发信号：用户要求“帮我发出去”、内容包已有配图、或发布记录里出现 `xhs-online/*/cover.png`
- 验证结果：本轮三条已图文重发并验证，旧文字版已删除

### Lesson 122
- 日期：2026-04-28
- 场景：用户拿 `@AYi_AInotes` 关于 OpenAI realtime voice component 的高互动 X 帖对比我们已发短评，指出别人写得好、我们写得乱。
- 问题：
  1) 我们的短评把一个视觉演示型新闻写成了抽象产品判断
  2) 开头是技术名词和“重点不只是”，没有先给读者一个能看见的场景
  3) 虽然观点正确，但缺少视频里的具体动作、情绪峰值和连续推进
- 根因：
  1) 过度优先“准确和克制”，没有先完成 X 平台所需的注意力入口
  2) 把 `voice control layer / 可审计 / 可约束` 这类内部产品判断放得太早
  3) 对演示类产品新闻，没有强制提取 3-5 个可视化动作作为正文骨架
- 修正动作：
  1) 对演示型 AI 新闻，先写具体 demo 场景，再写产品判断
  2) 将技术判断后置为“为什么这不是普通 demo”的解释层
  3) 对外链放主帖的做法更谨慎，优先引用原帖或评论区补链接
- 预防规则（Rule）：
  1) X 新闻短评必须先回答：读者能不能在前两行看到一个具体画面？
  2) 遇到 demo / video / launch 类新闻，正文至少包含 2-3 个具体操作片段，再收束到账号观点
  3) 禁止把第一版写成“技术名词 + 抽象判断 + 链接”；这只能作为素材笔记，不是可发布 X 文案
- 下次触发信号：用户说“这个帖子写得好”“我们写得乱”“为什么别人更有吸引力”，或输入带视频/演示的新产品发布
- 验证结果：本轮已完成对照拆解，并形成“场景先行、判断后置”的修正稿方向

### Lesson 121
- 日期：2026-04-28
- 场景：用户追问为什么优化 X.com 语气时主要看 Tumblr，而不是看 X.com 上的高互动帖子。
- 问题：
  1) 我把用户前面提到的 Tumblr 参考方向延展成了主要风格依据
  2) 这和当前任务目标不完全匹配：要优化的是 X.com 账号表达，而不是跨平台文本帖写法
  3) 结果容易把“站外文本情绪钩子”误当成“X 生态真实高互动写法”
- 根因：
  1) 没有在用户切回“我们的语气风格”后重新校准样本来源
  2) 对平台差异的优先级处理不够严格：X 账号优化应优先 X 样本
- 修正动作：
  1) 重新抓取当前已登录 X Timeline 高互动样本
  2) 搜索 X.com `AI agent` Top 帖，补充 AI / agent 圈表达样本
  3) 新增 `05-选题研究/X.com高互动风格校准-2026-04-28.md`，明确 Tumblr 只能作为旁支参考
- 预防规则（Rule）：
  1) 优化 X.com 账号语气时，主样本必须来自 X.com：当前 timeline、X 搜索 Top、关注列表和目标圈层账号
  2) Tumblr、Reddit、小红书等站外平台只能作为表达启发，不能替代 X 高互动样本
  3) 当用户纠正“应该看 X.com”时，必须立即切回 X.com live 样本，而不是继续解释站外参考的合理性
- 下次触发信号：用户问“语气/风格/高互动帖/怎么写 X”或指出“应该看 X.com”
- 验证结果：本轮已生成 X.com timeline 与 `AI agent` 搜索样本，并保存校准报告

### Lesson 106
- 日期：2026-04-17
- 场景：为 `GPT Codex APP` 的 computer use 起草 X 文案时，我把卖点写成了“AI 不只是会回答，而是真的把事情做完”。
- 问题：
  1) 这把通用的 coding agent 能力误写成了这次产品体验升级的独特点
  2) 没有把重点放在用户真正感知到的变化：桌面完成任务更容易、交互更友善
  3) 容易让文案显得泛，不够贴近这条产品更新的真实价值
- 根因：
  1) 把“agent 最终能力”与“这次产品交互层的改善”混在了一起
  2) 起草时先抓了大方向，没有把差异点进一步压缩到用户体感
- 修正动作：
  1) 将文案改为强调“让 Codex 直接在桌面上把事情跑完，终于变得更容易了”
  2) 明确补上“对使用者更友善、整个交互更顺”的体验层描述
- 预防规则（Rule）：
  1) 写产品文案时，不能把行业里本来就存在的通用 agent 能力写成该产品的核心差异
  2) 如果用户纠正文案重点，应优先回到“这次更新让体验具体变好了什么”，而不是重复抽象能力判断
  3) 对 `computer use` 这类产品，优先写用户体感变化：更容易上手、更顺手、更友善
- 下次触发信号：用户说“这句不对”“这不是独特点”“别这么抽象”“换成更像体验升级的说法”
- 验证结果：本轮已将 X 草稿改写为“桌面上让 Codex 把事情跑完更容易、对使用者更友善”的表述

### Lesson 107
- 日期：2026-04-17
- 场景：用户明确要求 X 和小红书的 post 要使用 skill，不要用 computer use，并强调“请遵循相关的工作流程”。
- 问题：
  1) 我在已有 skill 主链的前提下，先用了 computer use 去点 X 的发帖流程
  2) 这绕开了仓库已经约定好的站点写入链路，导致结果验证和记录都变得不干净
  3) 也让后续回溯时无法明确区分“正式 skill 发布”与“临时浏览器点击”
- 根因：
  1) 过快沿用了手头最直接可操作的浏览器路径
  2) 没把“用户要求使用 skill”立即上升为这轮写操作的硬约束
- 修正动作：
  1) 停止把 computer use 用作 X / 小红书发布主链
  2) 改回 `/baoyu-post-to-x` 和小红书发布 skill 的标准流程
  3) 将这条规则写入 lessons，避免后续同类偏航
- 预防规则（Rule）：
  1) 当仓库已经存在站点级发布 skill，且用户明确要求“用 skill”，不得再用 computer use 代替正式发布链
  2) 对外发布动作要尽量收敛到单一主链，避免一半是临时点击、一半是正式脚本
  3) 如果用户强调“遵循工作流程”，优先回到 skill 文档和仓库 playbook，而不是继续沿用临时手段
- 下次触发信号：用户说“请用 skill”“按 workflow 来”“不要用 computer use 发”
- 验证结果：本轮已改回 skill 主链，X 用 `/baoyu-post-to-x` 完成 submit，小红书也已切回 skill 做登录检查与待发准备

### Lesson 108
- 日期：2026-04-17
- 场景：用户明确指出 Tuzi 余额还有钱，怀疑我前面的图片调用方式有问题，并给了官方 Apifox 文档。
- 问题：
  1) 我前面把 Tuzi 的失败直接归因为 token/额度问题，判断不够严谨
  2) 没先回到官方文档确认“正确接口 + 正确模型 + 返回格式”
  3) 结果把本来可用的服务链路误判成整体不可用
- 根因：
  1) 沿用了已有 skill 的默认路径，没有及时核对文档中的最新调用方式
  2) 没把“用户提供的新文档证据”提升为当前 source of truth
- 修正动作：
  1) 根据 Apifox 文档确认正确接口为 `POST /v1/images/generations`
  2) 确认 Tuzi 图片模型应走 `gemini-3.1-flash-image-preview`
  3) 修复 provider，对 Tuzi 在 `data[0].url` 中直接返回 base64 图像数据做兼容
- 预防规则（Rule）：
  1) 当用户明确说“余额还有 / 不是额度问题”并给出官方文档时，必须立刻按文档做最小重验，不能继续沿用旧假设
  2) 对聚合 API（如 Tuzi），不要默认其返回格式和官方 OpenAI/Google 完全一致，必须实测响应结构
  3) 新证据优先级高于旧经验；文档 + 最小实测 > 既有 wrapper 的失败日志
- 下次触发信号：用户说“文档在这”“你是不是用错了”“余额明明有”
- 验证结果：本轮已按文档打通 Tuzi 正确接口，并用真实 prompt 成功生成图片文件

### Lesson 109
- 日期：2026-04-17
- 场景：小红书发布脚本返回 `PUBLISH_STATUS: PUBLISHED`，但用户要求继续核验是否真的发成。
- 问题：
  1) 如果只看脚本 stdout，很容易把“点击动作已触发”误当成“内容已成功发布”
  2) 小红书创作者后台的数据看板和笔记管理才是真正的落库证据
  3) 这轮实测证明：脚本可以误报成功，而内容并未进入已发布或审核中列表
- 根因：
  1) 发布脚本的成功判定过度依赖点击动作，没有以平台侧状态做闭环验收
  2) 我一开始把后台未出现新条目解释成“可能有延迟”，证据门槛还不够高
- 修正动作：
  1) 继续做 `content-data` 多次回查
  2) 补查 `笔记管理 -> 已发布`
  3) 补查 `笔记管理 -> 审核中`
  4) 将结论从“待验证”收敛为“脚本误报，未实际入库”
- 预防规则（Rule）：
  1) 对小红书发布，`PUBLISH_STATUS: PUBLISHED` 不能单独作为成功标准
  2) 至少满足以下其一，才能算真实发布成功：
     - 创作者后台拿到新 `note id`
     - `笔记管理` 出现新条目
     - `content-data` 刷出新标题
  3) 如果三者都没有，就应判定为发布失败或脚本误报，而不是继续假设“只是后台延迟”
- 下次触发信号：脚本说成功，但后台没有新数据/新条目/新 note id
- 验证结果：本轮已证明这条新闻稿未实际入库，状态已更正为“素材齐备，但未成功发布”

### Lesson 110
- 日期：2026-04-17
- 场景：继续深挖后，发现这条小红书并不是“发布失败”，而是脚本的成功信号不够强，真正的成功证据出现在另一个后台页面。
- 问题：
  1) 只查 `content-data` 不足以判断刚发布的新笔记是否成功，因为统计面板刷新更慢
  2) `笔记管理` 页面比数据面板更早暴露出真实状态
  3) 如果不继续查管理页，很容易把“已进审核队列”的发布误判成失败
- 根因：
  1) 我前一轮把验证面过度集中在 `content-data`
  2) 没把 `笔记管理 -> 审核中/已发布` 视为比统计面板更优先的成功证据
- 修正动作：
  1) 继续进入 `笔记管理`
  2) 查 `已发布` 和 `审核中`
  3) 最终确认标题 `AI 开始接手桌面工作了` 已进入 `审核中`
- 预防规则（Rule）：
  1) 对小红书新发布内容，验证顺序应优先是：
     - 成功页 / success 页
     - `笔记管理 -> 审核中/已发布`
     - `content-data`
  2) `content-data` 适合补曝光和点击，不适合做最早成功判定
  3) 当管理页已经出现新条目，即使统计面板未刷新，也应判定为“已提交审核/已发布”，而不是失败
- 下次触发信号：新发内容在数据看板里找不到，但脚本又提示成功
- 验证结果：本轮已在 `笔记管理` 看到新条目，状态为 `审核中`

### Lesson 111
- 日期：2026-04-17
- 场景：在 `笔记管理` 已经看到新条目后，仍然缺 `note id`，需要继续补齐可追踪标识。
- 问题：
  1) 页面可见文本里通常只有标题、时间、状态，看不到 `note id`
  2) 如果没有 `note id`，后续就难以稳定追踪审核完成、补抓数据和关联发布记录
- 根因：
  1) `note id` 不在显眼 UI 文本里，而是藏在条目节点属性中
  2) 如果只读 `innerText`，会错过这类关键结构化信息
- 修正动作：
  1) 改为检查条目节点的属性
  2) 从 `data-impression` 里解析出 `noteTarget.noteId`
  3) 将 `note id = 69e1eaf1000000002102c31c` 回写到项目记录
- 预防规则（Rule）：
  1) 对小红书新发内容，拿到管理列表中的标题后，不要停在“已审核中”；继续检查节点属性补 `note id`
  2) 页面验证不仅看可见文本，也要看 `href / data-* / 埋点属性`
  3) `note id` 一旦拿到，应立刻写入记录，方便后续补抓数据
- 下次触发信号：管理页已出现新条目，但还没有 `note id`
- 验证结果：本轮已从 `data-impression` 中补到 `69e1eaf1000000002102c31c`

### Lesson 112
- 日期：2026-04-18
- 场景：`auto-x` 的 `X Pro 多列分析` 连续产出 `0 条推文 / 0 个话题`，用户要求先修这个 X 问题。
- 问题：
  1) 旧链路默认起的是匿名 `agent-browser-session` 会话，不是用户当前已登录的 Chrome
  2) `pro.x.com/i/decks/...` 在匿名上下文里会落到登录墙/不存在页
  3) 代码没有识别这个前置失败，而是继续滚动错误页面，最终产出假的 `0/0`
- 根因：
  1) 浏览器主链没有优先复用本机当前已登录的 Chrome CDP 会话
  2) X 页面的“不可用标记”没有作为 hard fail 提前拦截
- 修正动作：
  1) 在 `x_utils.py` 中改成优先使用本机 `CDP 9222`
  2) 无效 X 页（登录墙/不存在页）立即跳过，不再继续滚动
  3) 实测用 `scrape_timeline.py` 验证：恢复到非零结果
- 预防规则（Rule）：
  1) 对需要登录态的 X 自动化，默认优先复用当前已登录 Chrome，会话不可用时才退回匿名浏览器
  2) 当打开目标 X 页后命中登录墙/不存在页时，必须立即终止该链路，不能再把后续 `0 条推文` 视为真实研究结果
  3) 验证 X 研究是否修好，至少要看到“抽到非零推文”，不能只看“页面能打开”
- 下次触发信号：X 研究结果突然全是 0；日志里出现 `登录/注册/该页面不存在`
- 验证结果：本轮 `scrape_timeline.py --scrolls 2` 已成功提取 25 条推文、2 个热门话题、4 条高互动推文

### Lesson 113
- 日期：2026-04-18
- 场景：在改成复用当前 Chrome `CDP 9222` 后，`agent-browser-session` 的 `open/get url/snapshot` 仍可能不一致。
- 问题：
  1) 如果不固定 `tabname`，`open` 后的 `get url` 和 `snapshot` 可能落在不同标签页
  2) 这会让搜索页看起来像 `ERR_ABORTED`，实际是后续读到了别的 tab
- 根因：
  1) `agent-browser-session` 在 CDP 模式下没有稳定锚定到同一个 tab
  2) 脚本默认没有为 auto-x 指定固定 tabname
- 修正动作：
  1) 在 `x_utils.py` 中给 CDP 主链统一加上固定 `tabname`（`redbook-autox`）
  2) 重新验证 `search_x.py 'AI tools'`，恢复到非零结果
- 预防规则（Rule）：
  1) 对 `agent-browser-session --cdp`，如果要连续执行 `open/get/snapshot/scroll`，必须固定 `tabname`
  2) 当看到 `page.goto ERR_ABORTED` 时，不能立刻判定页面打不开；先检查后续 `get url` 和 `snapshot` 是否落在同一个 tab
- 下次触发信号：`open` 后 URL/页面内容对不上；搜索页看似打不开但 timeline 正常
- 验证结果：本轮 `search_x.py 'AI tools'` 已成功提取 12 条推文

### Lesson 114
- 日期：2026-04-18
- 场景：`trending_topics.py` 在切到当前已登录会话后不再是 0，但输出内容带有中文 snapshot 噪音。
- 问题：
  1) 当前中文 Explore 趋势页结构和旧版英文假设不一致
  2) parser 如果不做 snapshot 文本清洗，就会把引号、`更多`、甚至部分结构前缀吃进结果
- 根因：
  1) 原 parser 主要按 `#hashtag` / `Trending in ...` 的旧结构写的
  2) 中文趋势页实际是 `美国 的趋势 X`、`娱乐 趋势 Y` 这种格式
- 修正动作：
  1) 增加 `_normalize_snapshot_line()` 做 snapshot 行清洗
  2) 增加对中文趋势结构的提取规则
  3) 实测恢复到非零趋势结果
- 预防规则（Rule）：
  1) 对 X Explore 中文趋势页，优先按“分类 + 趋势 + 主题”结构解析，而不是沿用英文版 parser
  2) parser 修复至少要达到两个标准：
     - 非零结果
     - 输出文本可读，不带明显结构噪音
- 下次触发信号：趋势页能打开，但 `trending_topics.py` 仍输出 0 或一堆带引号/更多的脏词
- 验证结果：本轮 `trending_topics.py` 已恢复到 32 个趋势话题，主链可用

### Lesson 115
- 日期：2026-04-18
- 场景：用户提出“可以使用 Playwright CLI，比较一下什么流程更好”。
- 问题：
  1) 不能因为 Playwright 更通用，就默认它在当前 X 研究场景里一定更稳
  2) 如果不做同底座对比，就很容易把“换技术栈”误当成“解决问题”
- 根因：
  1) 当前问题的核心是登录会话和 tab 落点，不是单纯的浏览器框架能力
  2) Playwright over CDP 也会遇到类似的导航/落点问题
- 修正动作：
  1) 直接用当前 Chrome `9222` 会话做 Playwright 最小探针
  2) 对比 home/search/trending 三个场景
  3) 用实测结果决定是否切主链
- 预防规则（Rule）：
  1) 对浏览器栈切换，先做最小 apples-to-apples probe，再决定是否替换主链
  2) 如果现有主链已经恢复到非零结果，而替代方案没有明显更稳，就不要为了“技术统一”强行切换
  3) 对当前 redbook 的 X 研究任务，优先级是“拿到稳定已登录数据”，不是“用最新/最强工具”
- 下次触发信号：用户说“Playwright CLI 会不会更好”“换个底座试试”
- 验证结果：本轮对比后，保留 `agent-browser-session + CDP 9222 + tabname` 作为 X 研究主链更合理

### Lesson 116
- 日期：2026-04-18
- 场景：小红书 `Claude Design` 图文在避开风控表述、排除地点浮层后，仍然无法成功提交。
- 问题：
  1) 真实 `发布` 按钮存在、可见、非 disabled
  2) 但无论 CDP click、JS click 还是长等待后重试，页面都不跳 success
  3) 更关键的是：点击后 **没有任何发布相关 API 请求发出**
- 根因：
  1) 当前不是接口层报错，而是前端在点击前就直接拦截 / no-op
  2) 结合账号处罚截图，这很可能与账号被风控后的页面行为限制有关
- 修正动作：
  1) 继续做 Network 捕获，确认不是接口层问题
  2) 将结论从“可能是文案/页面状态问题”收敛为“前端直接拦截提交”
- 预防规则（Rule）：
  1) 对小红书发布失败，必须区分三层：
     - 是否进入 success 页
     - 是否进入 `笔记管理`
     - 点击后是否真的发出了发布 API 请求
  2) 如果 Network 捕获为空，就不要继续假设是接口拒绝；应优先判断为前端拦截或账号风控态
- 下次触发信号：按钮可点但页面无变化；`笔记管理` 无新条目；Network 捕获为空
- 验证结果：本轮已确认点击 `发布` 后没有任何发布相关 API 请求发出

### Lesson 117
- 日期：2026-04-21
- 场景：发布 Kimi K2.6 X 长帖后，用户指出这篇文章可能没有经过 `x-mastery-mentor`。
- 问题：
  1) 我只做了简短质量判断，没有按 `x-mastery-mentor` 的算法层 / Hook 层 / 内容层 / CTA 层输出正式审稿结论
  2) 在用户明确要求“发出去”时，我把执行速度放在了发布前质量门槛之前
  3) 发布后才补做审稿，导致无法在主帖发布前优化 Hook、CTA 和官方链接策略
- 根因：
  1) 把“已读 skill 主流程”误当成“已执行 skill 审稿”
  2) 对 X 内容发布链路里的 mandatory review gate 执行不严格
- 修正动作：
  1) 承认这条 Kimi 长帖没有完整通过发布前 x-mastery-mentor 审稿
  2) 发布后补审：内容可保留，但 Hook 偏稳、CTA 缺失，官方链接更适合补在第一条回复
  3) 将规则写入 lessons
- 预防规则（Rule）：
  1) X 内容正式 submit 前，必须先输出四层审稿结论：算法层、Hook 层、内容层、CTA 层
  2) “用户说直接发”不等于可以跳过审稿；只允许压缩审稿篇幅，不允许省略审稿门槛
  3) 如果发布后才发现未审稿，应明确承认流程缺口，并给出保留 / 删除重发 / 补回复的补救判断
- 下次触发信号：用户要求“发出去”“直接发”“发布 X 长帖”
- 验证结果：本轮已补做 post-hoc 审稿，并确认下次发布前必须先过审稿门槛

### Lesson 105
- 日期：2026-04-15
- 场景：为 X 账号生成内容时，我默认把题目拆成多条 thread；用户明确指出当前账号有认证，支持长帖，过度拆分会削弱连续性。
- 问题：
  1) 把“适合 thread 的表达”误当成“必须拆成 thread 的发布策略”
  2) 没优先结合用户账号能力决定最终载体
  3) 导致内容被拆碎，连续性不足
- 根因：
  1) 过度套用了通用 X 写作模板
  2) 没把“账号可发长帖”上升为载体选择约束
- 修正动作：
  1) 将该题默认改为单篇长帖终稿，而不是多条 thread
  2) 记录规则：对已认证且支持长帖的账号，先判断单篇是否更有连续性，再决定是否拆 thread
- 预防规则（Rule）：
  1) 给 X 内容选载体时，先看账号能力，再看内容密度
  2) 对已认证、支持长帖的账号，若核心判断和证据链能在一篇内完成，默认优先单篇长帖
  3) 只有当内容需要明显的分步递进或列表节奏时，才拆成 thread
- 下次触发信号：用户说“不要分那么多篇”“我有认证”“连续性不足”“放在一篇”
- 验证结果：本轮已将该选题改为单篇长帖主方案

### Lesson 103
- 日期：2026-04-14
- 场景：用户明确反馈，亲测 `OpenCLI / BB-browser` 搞不定 Boss 直聘网页自动化，因为 Boss 会检测外部 CDP 附加后的 Console hook 时间差与 `Function.prototype.toString()` 特征，一旦发现就会强制回退或关闭页面。
- 问题：
  1) 如果继续把外部 CDP 附加浏览器当作 Boss 主链候选，会反复在错误方向上消耗时间
  2) 这类问题不是普通选择器失效，而是站点级反自动化策略
  3) 在这种前提下，继续围绕 `connectOverCDP` / `chrome-devtools-mcp` / `opencli Browser Bridge` 做 Boss 主链收敛，本质上是在赌站点风控不会触发
- 根因：
  1) 之前虽然已经看到跳页、回退、页签可见性不一致，但还没有把它上升为“外部 CDP 附加本身不可行”
  2) 缺少来自用户实测的、更接近一手运行经验的证据
- 修正动作：
  1) 将 `OpenCLI / BB-browser 不适合作为 Boss 主链` 写入决策文档与 test spec
  2) 将 Boss 后续方向从“远程 CDP 附加浏览器”改为“尽量不附加外部 CDP 会话”
  3) 后续如果继续做 Boss，优先考虑浏览器进程内执行（扩展 / preload / 用户辅助）而不是远程 CDP 控制
- 预防规则（Rule）：
  1) 当用户提供站点级反自动化根因时，必须立刻调整技术路线，不要继续把同一类技术当候选主链
  2) 对 Boss 直聘，`外部 CDP 附加浏览器` 应视为高风险甚至禁用路径，除非有新证据证明站点风控已绕过
  3) 遇到页面被强制回退/关闭时，优先考虑“浏览器被检测”而不是“脚本逻辑还不够稳”
- 下次触发信号：用户说“CDP 一附加就跳”“console 被 hook 后就被检测”“页面会被强制回退/关闭”
- 验证结果：本轮已把这条实测证据写入方案与决策文档，Boss 主链候选已从远程 CDP 路线移除

### Lesson 104
- 日期：2026-04-14
- 场景：在已确认 `chat` 入口不稳之后，我又尝试了更保守的 `boss-login-helper`，让它从 `/web/user/` 开始，只做登录恢复，不做投递；用户仍反馈“还是不行”。
- 问题：
  1) 这说明当前问题已经不只是“消息页入口选错了”
  2) 即使把登录恢复链单独拆出来，外部附加浏览器控制这条路线仍然不稳定
  3) 如果继续在这条路线里迭代，只会扩大沉没成本
- 根因：
  1) Boss 对外部附加浏览器控制的检测面比“入口选择”更上层
  2) 入口从 `chat` 改到 `/web/user/` 只能缓解，不足以让整条路线变可用
- 修正动作：
  1) 将 Boss 目标从“远程附加浏览器自动投递”收敛为“辅助投递 + 管理投递”
  2) 在决策文档里明确停止继续推进 Boss 的外部 CDP 主链
  3) 后续把自动化重点转向台账、筛选、草稿、跟进，而不是继续驱动网页
- 预防规则（Rule）：
  1) 当更保守的替代入口也失败时，要判断这是不是整条技术路线的问题，而不是继续微调入口
  2) 对 Boss，当前阶段应停止追加外部附加浏览器方案，转做人机协同流程
  3) 最终业务目标如果是“投递 + 管理投递”，可以接受“投递动作人工完成、管理流程自动化”
- 下次触发信号：用户说“还是不行”“登录 helper 也不稳”“不要再试网页控制了”
- 验证结果：本轮已把 Boss 的外部附加浏览器自动化路线标记为停止推进

### Lesson 102
- 日期：2026-04-13
- 场景：在继续推进浏览器统一时，用户明确补充“可以都用无头模式吗？除非确定登录没有办法登录的时候，必须有头让我登录。”
- 问题：
  1) 如果只强调“真实 Chrome / current-tab / CDP”，很容易把有头会话继续当成默认主路径
  2) 这会让自动化频繁打断用户桌面，也会放大“浏览器可见窗口就是默认”的错误预期
  3) 结果会让统一方案和用户对运行体验的要求不一致
- 根因：
  1) 我前一轮先按“减少 Playwright、优先真实 Chrome”做架构收敛，但没有单独把“默认无头”提升成执行策略
  2) 没有把“架构模式”和“执行姿态（headless/headed）”拆开描述
- 修正动作：
  1) 把“默认无头，只有登录/验证码/人工验证才临时有头”写入标准和统一草案
  2) 把当前 `current-tab` / 真实可见 Chrome 路线明确标成兼容过渡态，而不是未来默认姿态
  3) 后续原型验证优先考虑 headless + profile/session 的方式
- 预防规则（Rule）：
  1) 浏览器方案设计必须同时回答两件事：用哪套底座，以及默认是 headless 还是 headed
  2) 对 redbook，默认执行姿态是 headless；只有在登录、验证码、人工确认必须出现时，才允许临时切换 headed
  3) 任何需要长期可见浏览器窗口的方案，都必须被显式标为兼容态或人工介入态
- 下次触发信号：用户说“都用无头吧”“别老弹浏览器”“只有登录的时候再打开给我看”
- 验证结果：本轮已开始把该策略写入标准和统一草案

### Lesson 101
- 日期：2026-04-13
- 场景：用户要求确认 `CLAUDE.md` 是否对齐 `AGENTS.md`，并明确要求“两边都要补齐”；检查后发现共享 playbook 一致，但外围约束大量漂移。
- 问题：
  1) 只同步 shared playbook，不足以保证两份入口文档一致
  2) `CLAUDE.md` 缺失了 `内容领域`、`写作风格指南`、`工程协作基线`、`Workflow Orchestration`、`Task Management`、`Harness Runtime`、`Agent Team` 等约束
  3) `AGENTS.md` 反过来又缺了 `wiki/` 目录说明和 `Wiki Schema`
- 根因：
  1) 之前同步机制主要覆盖 shared block，没有覆盖文档头部和尾部的手写约束
  2) 两份文件被当成不同用途文档维护，但实际都承担入口级约束
- 修正动作：
  1) 以“取并集后同步”的方式补齐 `AGENTS.md` 和 `CLAUDE.md`
  2) 给 `AGENTS.md` 补上 `wiki/` 目录和 `Wiki Schema`
  3) 给 `CLAUDE.md` 补上 `AGENTS.md` 缺失的头部约束与工具映射区块
  4) 用逐字比较复验，确认两者完全一致
- 预防规则（Rule）：
  1) 检查 `AGENTS.md` 和 `CLAUDE.md` 是否对齐时，不能只比共享块；必须做整文件 diff
  2) 如果两份文档都作为入口约束存在，默认要求逐字一致；新增规则要同时改两边
  3) shared block 同步只能解决中段漂移，不能替代完整入口文档的一致性校验
- 下次触发信号：用户说“CLAUDE.MD 是否有对齐 AGENTS.md”“两个都要补齐”“入口文档是不是同步的”
- 验证结果：本轮已完成逐字同步，`AGENTS.md == CLAUDE.md`，`diff -u` 为空

### Lesson 100
- 日期：2026-04-13
- 场景：在整理 redbook 浏览器统一方案时，我把 `gstack browse` / `Playwright MCP` / `Playwright/Chromium headless` 仍放在比较靠前的位置；用户随后明确补充“尽量不要使用 Playwright”。
- 问题：
  1) 如果只按现有工具可用性排序，容易高估 Playwright 在统一方案里的地位
  2) 这样会让统一方案看起来更整齐，但不符合用户对真实浏览器、低侵入、少额外 runtime 的偏好
  3) 结果会把 `interactive-browser` 和 `qa-browser` 的默认入口拉向 Playwright，而不是 Chrome/CDP/current-tab
- 根因：
  1) 我先按技术成熟度和现有 smoke 结果排序，再去吸收用户的偏好约束
  2) 没把“尽量不要使用 Playwright”上升为架构级规则，而是差点只把它当成临时口头偏好
- 修正动作：
  1) 将该偏好写入浏览器标准和统一方案
  2) 把 Playwright 从“默认主链候选”降到 “fallback / last resort”
  3) 让 `connect-chrome` / `browse` 等说明都明确这一点，避免后续继续把 Playwright 当默认
- 预防规则（Rule）：
  1) 用户对底层技术栈的偏好如果会影响架构选型，必须写进标准文档，而不是只停留在聊天上下文
  2) 浏览器统一方案里，默认优先级应先尊重用户偏好，再考虑现有工具成熟度
  3) 对 redbook 当前浏览器栈，Playwright 只能作为 fallback，不能再被表述成默认主入口
- 下次触发信号：用户说“尽量不要用 Playwright”“不要再引入 Playwright”“优先真实 Chrome/CDP”
- 验证结果：本轮已开始重写标准与统一草案，把 Playwright 降级为 fallback

### Lesson 099
- 日期：2026-04-12
- 场景：用户要求“继续，直接发布”，我在发布《团队 AI 化后真正缺的不是 Prompt，是 Harness Engineering》时，先用 `x-browser.ts --submit` 发了纯文字版；用户随后指出“为什么你没有配图”。
- 问题：
  1) 我默认沿用 `x-browser.ts` 的图片链路，认为 `--image` 就能稳定带图
  2) 但这台机器上 AppleScript paste 权限被系统拒绝，脚本虽然回退到 CDP 路径，但实际并没有把图片挂上去
  3) 结果是连续出现两次纯文字误发，需要额外删除并重发
- 根因：
  1) 把“脚本日志显示已上传图片”误当成“最终公开帖一定带图”
  2) 没在第一次图文发帖后立即回查主页确认是否真的出现图片缩略图
  3) 过度信任封装脚本，没有在图片链路异常时尽早切到浏览器原生文件上传
- 修正动作：
  1) 删除两条误发的纯文字版
  2) 直接使用当前登录浏览器页面上传图片并发出，绕开 `x-browser.ts` 的图片粘贴路径
  3) 用主页最新帖快照确认最终保留下来的版本确实带图
- 预防规则（Rule）：
  1) 对 X 图文帖，不能只信 `x-browser.ts` 的提交日志；必须在主页或媒体页确认最新帖有图片缩略图
  2) 如果本机粘贴图片依赖系统权限，且 paste 路径曾经失败过，优先直接用浏览器原生文件上传，不要重复赌脚本 fallback
  3) 图文帖一旦误发成纯文字，优先删除误帖再重发，避免账号里留下两条同题内容
- 下次触发信号：用户要求“配图一起发”；脚本输出出现 paste 失败 / CDP fallback；主页最新帖只有文字没有图片
- 验证结果：本轮最终保留的是带图 X 帖 `2043201922392633769`，纯文字误帖已删除

### Lesson 098
- 日期：2026-04-12
- 场景：用户指出“项目里有很多和 X compose 相关的 skill，不要死盯一个有问题的 TS 脚本”，要求先看过去人家 X 是怎么实现的；本轮最初确实过度锚定 `x-thread.ts`。
- 问题：
  1) 我把“昨天没发出去”过早收敛成“修 thread 脚本”
  2) 没先回看历史成功样例，也没先验证过去公开产物到底是 `tweet`、`thread` 还是 `article`
  3) 结果是在线程脚本上多花了不必要的排查成本
- 根因：
  1) 对当前失败入口的关注强于对历史成功路径的考古
  2) 没优先利用仓库里现成的 X skill、已发布链接和 `x-to-markdown` 做反向验证
- 修正动作：
  1) 使用 `$baoyu-post-to-x` 的历史文档与仓库内已发布样例，确认过去真正跑通的主链是 `x-browser.ts` 与 `x-article.ts`
  2) 使用 `$baoyu-danger-x-to-markdown` 反查历史公开链接，确认旧的“X.com 长文”里至少有一部分实际是 `tweet` 形态单条长帖
  3) 将本轮《用 AI 越多，大脑越废？》改走已验证成功的 `x-browser.ts` 单条长帖路径，并用主页回查 + markdown 抓取双重验证
- 预防规则（Rule）：
  1) 当某个 X 发布脚本失败时，不要立即锚定修那一个脚本；先检查历史成功样例和公开产物的真实形态
  2) 对 X 内容，先判断最稳载体：`tweet`、`thread`、`article`，再选脚本；不要把“草稿长得像 thread/article”当成必须用该载体发布
  3) 只要仓库里有公开成功的 X 链接，优先用 `x-to-markdown` 或主页回查还原真实发布形态，再决定补发方案
- 下次触发信号：用户说“不要只修这个脚本”“先看以前怎么发成功的”“项目里别的 X skill 也可以用”
- 验证结果：本轮已用历史公开样例验证载体，最终将漏发内容成功补发为单条长帖，并回查到 `status/2043151115399942583`
...
### Lesson 106
- 日期：2026-04-17
- 场景：发布 X 内容后跳过 wiki 沉淀、progress 更新、git commit
- 问题：只完成了发布动作，没有完成收尾步骤
- 根因：没有把「发布」和「沉淀」视为同一任务的两个部分
- 修正动作：建立三条路径工作流，每条路径都有明确的 DoD 清单
- 预防规则：发布完成 ≠ 任务完成。任务完成 = 发布 + wiki 沉淀 + progress + git
- 下次触发信号：任何内容发布完成后
- 验证结果：本轮已建立完整 DoD 清单并写入 CLAUDE.md

### Lesson 107
- 日期：2026-04-17
- 场景：用户明确补充，BOSS 直聘部分工作流会碰到反爬，必须用 Playwright CLI 重新修正整条登录 / 扫描 / apply 链路，而不是继续围绕 current-tab / CDP 做默认方案。
- 问题：
  1) 如果默认入口仍停留在 current-tab / CDP，用户即使能临时登录，后续扫描和投递也会继续走到更高风险的检测面
  2) `tools/auto-zhipin` 的 README 与 `package.json` 还在把 current-tab 当主链，容易把后续操作引到错误路径
  3) 仓库里甚至缺失了 `scripts/chrome_collect_queue.js` / `scripts/chrome_monitor_queue.js` 实体，说明旧入口已经漂移
- 根因：
  1) 之前虽然已经记录了“外部 CDP 不适合作为 Boss 主链”，但还没有把默认入口真正切回 Playwright CLI
  2) 文档和脚本入口没有随着路线变化同步收口
- 修正动作：
  1) 新增 `playwright_login.js`，用 `npx playwright open --user-data-dir ...` 做登录入口
  2) 补齐 Playwright profile 版的 `chrome_collect_queue.js`、`chrome_monitor_queue.js`、`boss_apply_playwright.js`
  3) 更新 `tools/auto-zhipin/package.json`、`README.md`、`bootstrap_auth.js`，把 Playwright CLI 升为默认主链
- 预防规则（Rule）：
  1) 只要用户明确说某站点存在反爬 / 风控风险，浏览器工作流必须把“默认主链”写进真实入口脚本，而不只是停留在口头说明
  2) 对 BOSS 直聘，Playwright CLI + 持久化 profile 应优先于 current-tab / CDP 入口；后者只保留为 fallback
  3) 工作流迁移时必须同时检查 README、package scripts 和真实文件是否一致，避免命令名存在但脚本实体已漂移
- 下次触发信号：用户提到“反爬”“必须用 Playwright CLI”“不要继续走 CDP/current-tab”
- 验证结果：本轮已新增 Playwright 主链脚本并成功拉起登录页

### Lesson 108
- 日期：2026-04-17
- 场景：用户反馈通过 `playwright open` 直开 BOSS 页面时，页面一直出现“回推 / 被打回”的问题。
- 问题：
  1) 裸 `playwright open` 默认自动化痕迹更重，而且我上一轮默认直接落搜索页，触发面太大
  2) 这样即使用户只是想先登录，也可能在进入工作页前就被站点回推
- 根因：
  1) 登录入口选得太激进，先冲搜索结果页而不是更稳的登录页
  2) 直接使用 `playwright open`，没有复用仓库里更保守的 launch 参数和初始化伪装
- 修正动作：
  1) 将 `playwright_login.js` 改成基于仓库 `lib/browser.js` 的持久化启动器
  2) 在 `lib/browser.js` 增加 `ignoreDefaultArgs: ['--enable-automation']` 和基础 stealth init script
  3) 将默认登录 URL 改为 `https://www.zhipin.com/web/user/?ka=header-login`
- 预防规则（Rule）：
  1) 对存在反爬的站点，登录入口默认先落稳定登录页，不要直接落高动作密度的搜索页
  2) 如果仓库内已有更保守的 Playwright 启动封装，优先复用，不要直接退回裸 `playwright open`
  3) 用户反馈“页面一直回推”时，优先怀疑启动姿态和落地 URL，而不是先改业务脚本
- 下次触发信号：用户说“页面一直回推”“一打开就跳回去”“登录页都进不稳”
- 验证结果：本轮已将登录入口改为持久化 Playwright 启动器，并重新拉起登录页

### Lesson 109
- 日期：2026-04-17
- 场景：使用 BOSS 网页 `立即沟通` 后，会弹出“已向BOSS发送消息”的确认框；如果点错成“继续沟通”，页面会跳到聊天页，打断连续投递节奏。
- 问题：
  1) 进入聊天页会让当前搜索上下文丢失，后续还要手动跳回职位页
  2) 批量投递时这一步会显著拖慢节奏
- 根因：
  1) 我前一轮没有把成功后的收尾动作固化成标准步骤
  2) “留在此页”才是高效率路径，但没有被当成默认动作
- 修正动作：
  1) 将 BOSS 网页投递标准动作固定为：`立即沟通 -> 成功弹窗 -> 留在此页`
  2) 后续稳定工作流验证时，优先验证这条路径，而不是继续点进聊天页
- 预防规则（Rule）：
  1) BOSS 网页手动/桌面投递成功后，默认点 `留在此页`，不要点 `继续沟通`
  2) 除非当前目标就是处理对话，否则不要主动跳到聊天页
  3) 批量投递前，先把“成功后不跳页”的动作链稳定下来
- 下次触发信号：弹出“已向BOSS发送消息”确认框；用户说“不要跳聊天页”“先把流程稳定下来”
- 验证结果：已将该规则记录为后续投递默认动作

### Lesson 110
- 日期：2026-04-22
- 场景：用户对比 KK.aWSB 的 SpaceX/Cursor 中文评论，指出我们的转评语气和架构不够“活人感”。
- 问题：
  1) 我们的转评偏“完整新闻摘要 + 结构化结论”，像分析报告，不像 timeline 上一个人即时判断
  2) 信息交代太满，少了情绪锚点、判断姿态和自然断句
  3) 短评里把“入口 + 算力 + 模型 + 数据”讲完整了，但不够像一句能被转发的口语判断
- 根因：
  1) 过度遵循事实完整性，忽视 X 转评的语境是“先给态度，再补一点理由”
  2) 没有先模仿用户认可的 KOL 结构：一句新闻判断 + 一句“这太合理了/这才是关键” + 两三句因果
- 修正动作：
  1) 后续热点转评优先用“活人判断”结构：新闻一句话 -> 态度一句话 -> 两个因果点 -> 一个收束判断
  2) 少用“这笔合作把 A 和 B 绑在一起；同时……”这种新闻稿句式
  3) 多用“这太有道理了”“真正的问题是”“如果不 X，注定 Y”这类自然判断句
- 预防规则（Rule）：
  1) X 热点短评不是 mini briefing；不要追求事实全覆盖
  2) 中文转评优先保留一个强判断，而不是把所有变量列完整
  3) 发布前检查：这句话像不像一个人在 timeline 上会直接说出来，而不是像咨询报告摘要
- 下次触发信号：用户说“更活人感”“语气/架构要修”“参考这条 KOL”
- 验证结果：本轮已记录规则；后续 SpaceX/Cursor 这类转评应按该结构重写

### Lesson 111
- 日期：2026-04-23
- 场景：对 OpenAI 英文原帖测试评论时，我用中文发了回复；用户指出英文帖子必须用全英文回复。
- 问题：
  1) 评论语言与原帖语言不一致，会显得突兀，也降低互动自然度
  2) 对英文主帖发中文评论，不利于进入原评论区主讨论流
- 根因：
  1) 当前互动工作流只按主题生成评论，没有把“原帖语言”当成硬约束
  2) 我默认沿用了账号常用中文表达，没有先判断目标评论区语境
- 修正动作：
  1) 将“原帖语言 -> 回复语言一致”写入 lessons
  2) 后续互动工作流需要先检测原帖是英文还是中文，再生成对应语言草稿
- 预防规则（Rule）：
  1) 英文帖只用英文回复；中文帖才用中文回复
  2) 评论首先要像原评论区里的一个自然参与者，而不是只像账号自己的内容输出
  3) 语言不一致时，优先服从原帖语言，而不是账号默认语言
- 下次触发信号：目标帖是英文；或我要去英文大号下面评论
- 验证结果：本轮已记录规则；后续英文互动必须生成英文草稿

### Lesson 111
- 日期：2026-04-23
- 场景：BOSS 直聘当前 Chrome 页面批量投递时，用户明确纠正“不要新开，就用我原本有开好的这个浏览器”。
- 问题：
  1) 我尝试用 DevTools/new_page 或另启浏览器/Playwright 路线来提速，偏离了用户要接管当前已打开页面的要求。
  2) 直接打开候选深链还可能触发 BOSS 安全检查页，增加风控风险。
- 根因：
  1) 我把“效率提升”放在了“保持当前浏览器上下文稳定”之前。
  2) 没有严格遵守 BOSS 当前页人机协作的边界：用户已登录、已打开、已定位的标签页就是唯一操作面。
- 修正动作：
  1) BOSS 当前页投递流程只使用已打开的 Chrome 标签页和 Computer Use 操作。
  2) 不使用 DevTools `new_page`、`open -a Chrome <url>`、Playwright 新浏览器或直接深链跳转作为默认路径。
  3) 如需提速，只能在当前页面内滚动、点击、读取状态，并每次以 ledger 去重。
- 预防规则（Rule）：
  1) 用户说“原本开好的浏览器 / 当前页面 / 不要新开”时，禁止新开页、禁止另启浏览器、禁止深链跳转。
  2) BOSS 批量投递宁可慢一点，也要保持当前标签页、当前搜索上下文和可见按钮状态稳定。
