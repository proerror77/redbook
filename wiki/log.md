...
## [2026-04-29] ingest | Agent 入口迁移 X 长帖已发布

来源：`01-内容生产/02-制作中的选题/2026-04-29-AI-Agent三选题内容包/x-post-agent入口迁移.txt` + Chrome `Shih` profile 发布验证

触及页面：4个
- `01-内容生产/02-制作中的选题/2026-04-29-AI-Agent三选题内容包/发布记录.md` — 记录状态 URL、发布时间、profile 诊断和验证证据
- `01-内容生产/02-制作中的选题/2026-04-29-AI-Agent三选题内容包/发布清单.md` — 将 Agent 入口迁移标记为已发，剩余 X 选题仍待发
- `tasks/progress.md` — 记录本轮 X 发布验证和小红书暂停状态
- `tasks/lessons.md` — 新增 Shih profile 才是 X 发布验证默认会话的规则

关键洞察：
- 本轮发布强证据是 Shih profile 个人主页顶部已出现新帖，状态链接为 `https://x.com/0xcybersmile/status/2049311953957634056`。
- 登录本身没有问题；问题是最初验证/脚本链路使用了独立自动化 profile，不能代表用户当前 Shih 会话。
- 小红书本轮不继续提交，保留本地草稿和渲染素材，等待用户手动或后续低风险窗口处理。

## [2026-04-29] incident | X 三帖已发但漏图

来源：用户反馈“一个严重的错误，都没有图。” + Chrome `Shih` profile 主页验证 + 本地 `xhs-online/*/cover.png` 文件检查

触及页面：4个
- `01-内容生产/02-制作中的选题/2026-04-29-AI-Agent三选题内容包/发布记录.md` — 记录三条状态 URL、漏图事故和修复建议
- `01-内容生产/02-制作中的选题/2026-04-29-AI-Agent三选题内容包/发布清单.md` — 补入每条应附图片路径与图文重发包
- `tasks/lessons.md` — 新增“存在 cover/card 资产时不得纯文字发布”的规则
- `tasks/progress.md` — 记录事故状态和待确认动作

关键洞察：
- X 发布不能只以文案文件为发布输入；同一内容包下若存在 `cover.png` / `card_*.png`，必须进入图文发布链。
- 自动化后续修复要把“有图未传”作为阻断条件，而不是发布后人工发现。
- 三条文字版旧帖已确认存在，后续正确修复路径是用户确认后删除旧帖并图文重发。

## [2026-04-29] ingest | X 三帖图文重发与旧帖删除完成

来源：Chrome `Shih` profile / `@0xcybersmile` 主页验证 + X 删除提示 + 本地发布记录更新

触及页面：4个
- `01-内容生产/02-制作中的选题/2026-04-29-AI-Agent三选题内容包/发布记录.md` — 回写三条图文重发 URL、旧文字版删除状态和验证证据
- `01-内容生产/02-制作中的选题/2026-04-29-AI-Agent三选题内容包/发布清单.md` — 将 X 状态改为已图文重发并验证
- `tasks/lessons.md` — 新增 X 图文发布必须验证正文 + 图像链接的规则
- `tasks/progress.md` — 记录本轮修复完成状态

关键洞察：
- X 发布完成标准必须是状态链接、正文和 `图像`/`photo/1` 同时成立；`你的帖子已发送` 只能作为中间信号。
- 修复发布事故时，应先发出并验证替代新帖，再删除旧帖，避免短窗口内内容完全消失。
- 本轮最终三条图文帖为：
...
## [2026-05-09] ingest | 企业导入 AI Ready 长文发布

来源：`01-内容生产/02-制作中的选题/2026-05-09-企业导入AI不是买模型而是重构管理系统/` + X Article 发布回读

触及页面：3个
- `wiki/选题/AI Agent企业导入与协作.md` — 增加 AI Ready 定义、1-3 个流程试点路径、三阶段企业服务切入和已发布 X Article URL
- `01-内容生产/02-制作中的选题/2026-05-09-企业导入AI不是买模型而是重构管理系统/发布记录.md` — 记录 X Article URL、发布时间和图片持久化证据
- `tasks/progress.md` — 记录 Codex Image 纠偏、X Article 发布和平台侧验证结果

关键洞察：
- 企业导入 AI 的第一判断不是有没有模型，而是流程、数据、权限、评估、治理是否能让 AI 接管一部分工作。
- X Article 正文插图必须以内联图片发布；普通 X gallery 不能替代长文正文配图。
- 发布验证要回读公开状态页和平台侧图片资源；本轮状态页检测到 4 张 `pbs.twimg.com/media` 图片。

## [2026-05-11] ingest | 每日研究报告自动摄入
...
- `wiki/选题/低token-本地AI-端侧模型.md` — 追加今日信号
- `wiki/选题/AI工具信任与AI Slop.md` — 新建选题页
- `wiki/index.md` — 更新索引、日期和总页面数
...
## [2026-07-17] ingest | 每日研究报告自动摄入

来源：05-选题研究/ X/HN/Reddit 每日报告；以及 `docs/reports/grok-research-2026-07-17.md`（READY）

触及页面：11个
- `wiki/选题/AI Agent企业导入与协作.md` — 追加今日信号
- `wiki/选题/AI工具与效率.md` — 追加今日信号
- `wiki/方法论/跨平台账号编排.md` — 追加今日信号
- `wiki/素材/框架库.md` — 追加自治等级与控制匹配框架
- `wiki/素材/案例库.md` — 追加企业 agent 平台与规模化价值案例
- `wiki/素材/金句库.md` — 追加企业 agent 治理表达
- `wiki/概念/Agent身份与权限.md` — 新建并写入今日信号
- `wiki/概念/Agentwashing.md` — 新建并写入今日信号
- `wiki/概念/Agent自治等级.md` — 新建并写入今日信号
- `wiki/概念/Agent记忆治理.md` — 新建并写入今日信号
- `wiki/index.md` — 更新索引日期、分类和总页面数

关键洞察：
- 企业 agent 的竞争中心从聊天能力转向身份、网关、审批、审计、回滚和记忆生命周期。
- agent 采用率、预算增长与企业级 ROI 并不等价；流程重设计、信任和停损标准必须单独验证。
- 治理应按 Observe、Advise、Act with Approval、Act Autonomously 分级，自治等级越高，控制与运营证据越重。
