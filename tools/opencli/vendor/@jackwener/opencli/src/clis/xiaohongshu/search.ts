    ...
    const pageState = await page.evaluate(`
      (() => ({
        hasLoginGate:
          ...
          !!document.querySelector('[class*="login"], [class*="Login"]'),
      }))()
    `);

    if (pageState?.hasLoginGate) {
      throw new Error(
        'HTTP 401. Are you logged into www.xiaohongshu.com? Search results require a public-site login; creator login is not enough.'
      );
    }
