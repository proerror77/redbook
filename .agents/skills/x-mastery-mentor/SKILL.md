...
Step 2: 采集近100条推文数据
  默认 research-only：只读采集、整理、诊断；不得发布、回复、关注、删除或修改账号资料。
  按优先级依次尝试，每种方式失败后自动切到下一种：

  方式1（首选）：Redbook 当前登录浏览器 / Chrome Extension / OpenCLI 只读采集
    → 先运行 `tools/redbookctl browser` 查看是否已有可复用 X tab / CDP
    → 优先使用真实已登录 Chrome profile 的只读接口或 opencli timeline/user 数据
    → 不新开发布 composer，不输入文本，不点击 reply/post/follow
    → 采集每条推文的：
      文本、likes/retweets/replies/bookmarks/views、时间、媒体类型
    → 失败判定：无登录态/接口不可用/样本少于50条 → 切方式2
  ...
  方式3（兜底）：computer-use 视觉只读
    → 仅在结构化浏览器接口不可用时使用
    → 打开 https://x.com/{username}
    → 截图确认页面加载成功
    → 逐屏滚动（每次scroll后等2秒），截图提取数据
    → 失败判定：页面显示登录墙/404/超时3次 → 切方式4
