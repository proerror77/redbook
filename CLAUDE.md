...
- Social media 资料收集与写作闭环：`tools/redbookctl social-loop status` / `tools/redbookctl social-loop run --step collect` / `tools/redbookctl social-loop review` / `tools/redbookctl social-loop next`（research-only；不发布、不评论、不回复、不关注）
