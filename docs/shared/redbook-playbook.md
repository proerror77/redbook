...
- 每日 X 运营必须先生成 `X-timeline-fresh-following-YYYY-MM-DD.md` / `.json`：从当前 X following chronological timeline 读取目标 100 条，并按当天日期过滤，作为“今天发生了什么”的第一证据；`X-timeline-sample-YYYY-MM-DD.md` / `.json` 的 home/for-you 样本只作补充；再生成 `X-互动队列-YYYY-MM-DD.md`，从样本里筛 20 条高互动人/帖和回复草稿。默认只生成候选和草稿，不自动发布评论。
...
2. 如果用户要求看 X timeline，必须优先读取或生成 `05-选题研究/X-timeline-fresh-following-YYYY-MM-DD.md`：默认目标 100 条当前 following chronological timeline，并按当天日期过滤；样本不足时必须在结论里标明缺口，不能把 home/for-you 样本或 20 条互动队列当成完整 today timeline 观察。
3. 浏览器姿态默认 headless，只有登录、验证码或明确视觉确认才 headed。
4. fail-closed：X timeline / X search / X Pro 不可用时，必须明确说 X 证据缺口；不得用发布提醒、制作中旧稿、待深化旧题或历史选题池冒充“今天值得写”。
5. 发布提醒只用于提醒旧稿可发布，不是当日选题来源；除非用户明确要求“从旧稿里挑今天发什么”。
6. 输出 3-5 个可写选题，每个使用 `选题决策卡`：热度、争议点、账号契合、建议形态、推荐角度、目标受益人、Persona 匹配、为什么不建议其他形态。
7. 对每个候选明确平台翻译：X.com 的观点钩子是什么，小红书能否落到企业/商业/管理者场景；不能落地的小红书只标 X，不强行多平台。
8. 同步生成每日 X 互动队列：优先从 fresh following timeline 样本中挑有回复/转帖/喜欢势能、且能接上账号主线的人和帖子，生成 20 条回复候选；实际发布回复时必须按 `x-engagement-reply-workflow` 复筛和审稿。
9. 不创建完整 content run，不写发布清单，不自动写 `00-选题记录.md`。
...
- 每日 X timeline 样本与互动队列：`tools/redbookctl daily` 默认先生成目标 100 条、按今天过滤的 `X-timeline-fresh-following-YYYY-MM-DD.md`，再生成补充 `X-timeline-sample-YYYY-MM-DD.md` 和 20 条 `X-互动队列-YYYY-MM-DD.md`；单独调试 fresh following 用 `node tools/auto-x/scripts/build_fresh_following_sample.mjs --date YYYY-MM-DD --target 100 --min-fresh 80`
