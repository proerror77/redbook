...
这个 Skill 是 BOSS 工作流的入口说明。当前主链是 `tools/auto-zhipin` 的 Playwright CLI + 持久化 profile；current-tab/CDP 作为低层 fallback 或诊断入口。
...
`boss:apply-current` 可在 Playwright profile 主链失效或需要低层诊断时使用；`boss:apply-opencli` / OpenCLI / OpenSeal 只保留为版本和 doctor 诊断参考，不用于 BOSS chat、search 或 apply 主流程。
