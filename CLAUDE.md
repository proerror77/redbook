...
- `Research Agent`：query wiki（`wiki/选题/` + `wiki/方法论/` + `wiki/素材/`）、历史文稿，产出证据清单
- `Writing Agent`：根据输入生成初稿/改稿，保证观点与可读性
- `Platform Agent`：将文稿改写为小红书/抖音/X.com/公众号版本
- `QA Agent`：自检清单（事实/结构/AI痕迹/平台规则），不是子 agent 调用
...
### 6. 质量与边界
- 不跳过 wiki query 直接写稿（先读 `wiki/index.md` 找相关页面）
- 未经自检清单（事实/结构/AI痕迹/平台规则）不得进入发布步骤
...
执行约束：
- 先 query wiki（`wiki/选题/` + `wiki/方法论/` + `wiki/素材/`），再开始创作
- 每步给出简短进展
- 完成前必须做自检清单（事实/结构/AI痕迹/平台规则）与可发布性检查
```
