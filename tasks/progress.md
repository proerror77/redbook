...
## [2026-09-07] BOSS 0.18.7 过期标签请求与暂停范围修复

- 现场 Gate 再次出现 manual_direction_not_verified；代码确认 sessionStorage 中过期 requestedKey/requestedAt 使每次恢复再次全局暂停。
- 过期请求转为目标标签恢复并清除；标签失败改为本地 30 秒退避、最多 3 次，用户可信点击重置；不再写全局风控暂停。允许相邻标签列表重叠，以选中状态和非空稳定列表检查，保留投递前目标复核。
- 98/98 用户脚本与 Gate 测试通过，包含过期请求清除、失败次数上限、非全局暂停、相同列表切换；语法及差异检查通过。静态源回读 0.18.7。
- 尚需用户更新加载新版再解除旧服务器暂停；未操作浏览器、未宣称真实页面已恢复。

## [2026-09-07] BOSS 0.18.6 成功回写幂等与未知结果去重

- Store 成功记录保持终态，Gate 对同 URL/身份成功重放不更新投递时间；历史点击未验证失败按 URL/身份阻止重投，保留点击前失败可重试。
- 油猴区分待终审、可投、已沟通、结果待确认；结果未知后暂停新投递。
- 完整回归 359/359 通过，差异检查通过。Gate 已更新并按用户启动授权恢复；回读允许、无暂停、27/150。静态源为 0.18.6。
- 已有画面记录最新仍显示 0.18.5（13:05:05），因此未确认浏览器已加载新版或新增投递。需用户更新刷新；未操作浏览器，未修改历史投递结果。

## [2026-09-07] BOSS 0.18.5 标签意图与跨页证据修正

- 修复刷新或返回默认推荐时覆盖保存标签的问题：可信用户点击记录选择意图，初始化/重载按保存位置恢复，恢复同方向不清零计数，跨天保留位置；手动点击推荐可明确覆盖旧位置。
- 移除聊天 URL 作为成功证据的两个运行路径及旧辅助判断；旧 URL-only 成功待回写不会重放。未确认结果保留并暂停新投递，已确认结果仍在账本应答后清除，未修改历史投递账本。
- 113/113 聚焦测试通过，覆盖默认推荐恢复、可信/合成点击、手动推荐、聊天跳转误报、旧待确认结果及回写重试；语法和差异检查通过。更新源回读 0.18.5，Gate 在线且无暂停。
- 尚未操作或验证浏览器新实例；需要用户更新油猴并刷新。账本回读 27/150 是既有记录，未据此宣称本轮新增投递已验证。

## [2026-09-05] BOSS Copilot 0.18.4 标签与消息维护衔接

- 将标签确认、轮换与消息维护归入单个扫描流程；去除启动阶段并行延时切页。按当前选中标签及页面顺序轮换，切换须验证列表更新，失败不推进游标。
- 每轮标签处理后进入三分钟可见会话维护并返回保存标签；日额度到达后安排一次整理；手动标签切换会被采纳，临时聊天返回保留本方向计数。范围仍为已加载的最多 30 个会话，普通回复仅生成草稿。
- 112/112 聚焦测试通过，涵盖四标签轮换、点击失效、维护回执、手动选择及返回计数；语法与差异检查通过。静态服务回读 0.18.4，Gate 允许投递且无暂停，今日记录 0/150。
- 未操作浏览器，尚需用户更新 Tampermonkey 并刷新后验证真实页面标签标记和投递/维护闭环；运行目录既有混合改动未提交。

## [2026-09-05] BOSS Copilot 0.18.3 运行逻辑修正

- 独立运行目录 `/Users/proerror/Library/Application Support/boss-auto-zhipin`：暂停时保留本地健康轮询；观察到服务器暂停再恢复且页面无风险后重启扫描；跨天重置页面计数。
- 风控文本剔除不可见节点及普通卡片/JD；目标详情“继续沟通”代替全页计数；已有沟通弹窗阻止新点击，额度提醒不再单独证明成功；每次复核重新读取 JD。
- 完整 JD 语义规则增加独立方案、架构、端到端交付或业务落地责任要求，保留 AI 应用工程师/FDE 范围，不按职称或年限硬拦。提示词参与 reviewKey，旧判定不会按旧键复用。
- 验证：111/111 聚焦测试通过，语法及差异检查通过；静态服务提供 0.18.3，Gate 已重启并回读允许投递、无风险限制、今日 0/150。
- 尚未验证浏览器加载 0.18.3 或新增真实投递；用户需在 Tampermonkey 更新后手动刷新。未操作浏览器。运行目录有既有混合修改，未提交该目录。
...
- FDE 详情页返回“工作经历不匹配”，未计入成功；随后发现薪资上限例外会放行 `40–70K·13薪`，已撤掉该例外，改为只按年化最低月均判断。后续批次暂停至修复后的 server 生效。

## [2026-07-11] 修正年化薪资 gate

- gate 现在解析 `·N薪`，按 `最低月薪 × N ÷ 12` 计算年化月均；`企业AI落地（FDE）大客KA合伙人` 的 `50–80K·20薪` 计算为最低约 `83.3K/月`，已重新纳入 allow。
- 增加薪资年化回归测试；相关 filters/gate 测试通过。
...
**完成了什么：**
- 按用户要求从当前 X timeline 扩量回复 50 个此前未回复过的 source status。
- 执行固定 workflow：浏览器/CDP 预检、`x-login` 账号预检、排除低质量候选、手写 reviewed JSON、内容复审、脚本 `review-only` gate、1 条 live smoke、完整批次、失败复核与补发。
- 内容只保留 AI agent、Codex/Claude Code、工具链、企业落地、权限/密钥/风控、预测市场/交易风险等账号主线相关帖子；剔除政治冲突、擦边娱乐、纯喊单和低上下文段子。
- 主批次中 4 条是提交后延迟回读，后续从 `with_replies` 或源帖页面找到验证 URL；@fxtrader 因 CDP 超时单独 retry 后成功。

**验证 / 证据：**
- `tools/redbookctl browser --json`：X/CDP `http://127.0.0.1:9224` 可复用。
- `tools/redbookctl x-login --timeout-ms 45000`：账号 gate 通过，发布账号为 `Smileyface @0xcybersmile`。
- `reply_engagement_queue.mjs --review-only --limit 50`：`approved=50 blocked=0`。
- 合并 verified 记录：`05-选题研究/X-互动回复记录-2026-05-05-batch4-50-verified.md` / `.jsonl`。
- 最终统计：`success=50`、`failed=0`、`unique_sources=50`、`unique_verify_urls=50`。
- 首条验证 URL：`https://x.com/0xcybersmile/status/2051634299087270395`；末条验证 URL：`https://x.com/0xcybersmile/status/2051640357465845849`；retry 验证 URL：`https://x.com/0xcybersmile/status/2051641089313083447`。

**遗留 / 注意：**
- 大批量回复中 `with_replies` 偶发延迟，后续继续保留“最终记录 + 延迟回读复核 + 必要时 retry”的收口步骤。
- 不要只看主批次 stdout；本轮主批次 stdout 是 `45/5`，但最终合并验证后才是 `50/0`。
...
**完成了什么：**
- 确认 Chrome `Shih` profile 是当前 X 正确登录会话，对应账号 `@0xcybersmile`。
- 确认 X 登录状态正常：左侧有发帖入口，账号菜单显示 `Smileyface @0xcybersmile`，无登录页、验证码或账号挑战。
- 验证 `Agent 入口迁移` 已发布到 X.com。
- 状态链接：`https://x.com/0xcybersmile/status/2049311953957634056`
- 发布记录已写入 `01-内容生产/02-制作中的选题/2026-04-29-AI-Agent三选题内容包/发布记录.md`。

**未完成 / 遗留：**
- 小红书没有继续发布；本轮停在本地草稿与渲染素材阶段，避免继续触发风控。
- 剩余两个 X 选题尚未发布：`Claude 进入创意工具`、`AI 裁员陷阱`。

**下次会话优先做：**
- 若继续 X 发布，必须优先使用 Shih profile 的已登录 Chrome 会话验证，而不是自动化脚本默认 profile。
- 小红书等风控冷却后再由用户手动发布或重新接管。
...
## [2026-04-29] 会话摘要：AI Agent 三选题内容包

**完成了什么：**
- 按用户指定写完选题 1 / 3 / 5：
  - Claude 进入 Blender / Adobe / Canva 等创意工具
  - AI Layoff Trap 与 AI 裁员机制
  - Agent 入口从聊天框迁移到邮箱 / 浏览器 / 本地代码图谱 / 语音 / 可穿戴设备
- 为每个选题产出 X 主帖版本、第一条回复、推荐发布版本。
- 为每个选题产出小红书本地草稿，包含标题、封面、图文页结构、caption、标签和手动发布备注。
- 完成 x-mastery-mentor 自审、QA 报告、发布清单和 Wiki 沉淀。

**产出文件：**
- `01-内容生产/02-制作中的选题/2026-04-29-AI-Agent三选题内容包/`
- `01-内容生产/02-制作中的选题/2026-04-29-AI-Agent三选题内容包/小红书草稿/`

**验证：**
- 小红书草稿只保存在本地文件，未调用发布工具，未发出。
- X 文案未发布，等待用户明确确认。
...
## [2026-05-07] 删除 Cloudflare + Stripe 短评

**完成了什么：**
- 用户指出 Cloudflare + Stripe 不是足够新的今日内容，不应作为今日短评发布。
- 已删除：`https://x.com/0xcybersmile/status/2052260091349680606`。
- 已把内容包状态从 `published` 改为 `deleted`，并在发布账本追加删除记录。

**验证：**
- 删除前独立状态页确认帖子存在，作者和正文可见。
- 删除操作通过状态页主帖更多菜单执行，并完成二次确认。
- 删除后重新打开原 URL，页面显示 `唔...该页面不存在。请尝试搜索别的内容。`
- 正文 `Cloudflare + Stripe 这个更新` 不可见。

**复盘：**
- 这次选题问题在于：HN 当日热度不等于内容本身是今日新信号。后续“今日选题”必须区分 `今日被讨论` 和 `今日发生 / 今日新发布`，旧新闻只适合收藏、复盘或作为长文论据，不应当作今日短评。
...
## [2026-08-24] BOSS 薪资门槛放宽到区间上限 30K

- 用户明确要求 `20-40K` 不再因下限 20K 被挡；共享 `evaluateJob()` 已从年化下限判断改为年化上限判断。
...
- 2026-08-31：补齐油猴交付版本号：静态源此前仍是 `0.18.0`，Tampermonkey 因此不会替换已安装脚本；现已升至 `0.18.1`，并回读静态源含地点兜底。聚焦测试 67/67、语法检查、Gate `/health` 均通过。
