...
## Base Broth + Style Seasoning

Use the 5D system as implementation details, but start with a reusable cover recipe:

- **Base broth**: stable account-level cover logic: understand why the object is worth seeing, then make one clear recommendation reason visible in 3 seconds on mobile.
- **Style seasoning**: one reusable style module that shapes the cover. Examples: blueprint order, product manual, cinematic title, eastern literati, exhibition wall, garden journal, variety preview.
- **Content variables**: title, object, reader promise, platform ratio, one or two topic-specific visual elements.

When building the prompt, convert the selected seasoning into concrete constraints:

- layout grid and title placement
- typography feel and text density
- accent color and background material
- illustration / object relationship
- forbidden decorative habits

Do not mix many seasonings. One main seasoning is the default; add a secondary seasoning only when it clarifies the recommendation object.
